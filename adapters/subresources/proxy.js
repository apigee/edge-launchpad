...
function build(context, resourceName, subResourceName, params, cb) {
	...
	async.eachSeries(items, build_proxy, function(err){
		if(err){
            ...
            lib.print("info","exit building proxy");
			cb();
		}
		...
		if (err) {
			...
			var npm_dir = path.join(proxy_target_dir, 'resources/node')
			 ...
			 if (fs.existsSync(npm_dir)){
                 ...
                 //
                 //})
                 lib.print("info","folder exists: " + npm_dir);
                 gulp.src(""+npm_dir+"/package.json").pipe(install());
                 gulp.src(""+npm_dir + '/node_modules/*').pipe(zip('node_modules.zip'));
			 } else {
                 lib.print("error", "folder "+ npm_dir + " does not exist");

			 }
            lib.print("info","before callback in buildproxy:"+item.name);
			callback();
		}
		/*
...
function deploy(context, resourceName, subResourceName, params, cb) {
    ...
    lib.print("info", "after creating items[]");
